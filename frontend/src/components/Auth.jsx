  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const payload =
        mode === "login"
          ? { email, password }
          : { name, email, password, role: normalizeRole(form.role) };

      const data =
        mode === "login" ? await api.login(payload) : await api.signup(payload);

      if (!data?.user || !data?.token) {
        throw new Error("Invalid auth response from server");
      }

      // Persist and notify app; App.jsx will redirect.
